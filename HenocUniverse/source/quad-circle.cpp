 ...
 * TT102 HENOC 
 * 2008 @ ESCOM-IPN
 * Intersecciones cuadrado-circulo y generador de contactos
 */
...
bool HenocUniverse::Intersection::TestCircleQuad(const Geometry &c, const Geometry &q){
	return TestQuadCircle(q, c);
}

bool HenocUniverse::Intersection::TestQuadCircle(const Geometry &g1, const Geometry &g2){
...
void HenocUniverse::Intersection::FindCircleQuad(const Geometry &c, const Geometry &q, ContactList &contacts){
	contacts.ToggleNormalInversion();
	FindQuadCircle(q, c, contacts);
	contacts.ToggleNormalInversion();
}

void HenocUniverse::Intersection::FindQuadCircle(const Geometry &g1, const Geometry &g2, ContactList &contacts){
	assert(g1.GetShape() == Shape::Quad);
	assert(g2.GetShape() == Shape::Circle);
	const Quad &q = static_cast<const Quad&>(g1);
	const Circle &c = static_cast<const Circle&>(g2);
