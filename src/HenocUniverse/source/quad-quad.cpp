 ...
 * TT102 HENOC 
 * 2008 @ ESCOM-IPN
 * Intersecciones cuadrado-cuadrado y generador de contactos
 */
...
bool HenocUniverse::Intersection::TestQuadQuad(const Geometry &g1, const Geometry &g2){
...
void HenocUniverse::Intersection::FindQuadQuad(const Geometry &g1, const Geometry &g2, ContactList &contacts){
    assert(g1.GetShape() == Shape::Quad);
    assert(g2.GetShape() == Shape::Quad);
    const Quad &a = static_cast<const Quad&>(g1);
    const Quad &b = static_cast<const Quad&>(g2);
